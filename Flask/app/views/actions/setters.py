...
import io
import re
from app.controller.send_email2 import send_email
from app import *
...
def upload_existing_project():
    ...
    try:
        ...
        logger.log(LOG_LEVEL, 'Error: {}'.format(str(e)))
        raise
...
def send_comment():
    ...
    if main.IsLogin():
        ...
        if db.connect(db_adapter):
            ...
            if result:
                all_people_tagged = re.findall('@[A-za-z@.]+', request_data['comment'])

                if len(all_people_tagged) > 0:
                    for person in all_people_tagged:
                        person = person.replace('@', '')
                        message, user = db.get_user(db_adapter, {'username': person})
                        if message['code'] == 200 and user['email']:
                            body = 'You are tagged in a new comment posted by @' + session['user']['username'] + '!\n\n'

                            ic = db.fetch_ic(db_adapter, request_data)

                            body += 'Project: ' + request_data['project_name'] + '\n'
                            body += 'Path: '
                            body += ic.path
                            body += '' if ic.is_directory else ic.type
                            body += '\n\n'

                            body += 'The comment is as follows:\n'
                            body += request_data['comment']

                            send_email("[CDE] You are tagged in a new comment!", body, [user['email']])
            ...
            if result:
                ...
                if len(all_people_tagged) > 0:
                    for person in all_people_tagged:
                        ...
                        message, user = db.get_user(db_adapter, {'username': person})
                        if message['code'] == 200 and user['email']:
                            body = 'You are tagged in an updated comment posted by @' + session['user']['username'] + '!\n\n'
                            ...
                            body += 'The updated comment is as follows:\n'
                            body += request_data['comment']

                            send_email("[CDE] You are tagged in an updated comment!", body, [user['email']])
